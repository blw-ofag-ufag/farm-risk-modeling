#' =============================================================================
#'       title:  PREPARATION OF A LARGE INTEGRATED ML DATASET FOR THE FOAG
#'      author:  Damian Oswald
#'        date:  2025-06-02
#' description:  Pulls raw data from the FOAG production systems AGIS (struct-
#'               ural attributes), ACONTROL (on-farm inspections) and HODUFLU
#'               (manure and recycling-fertilizer flows), harmonises keys,
#'               resolves duplicates, and assembles a tidy, de-identified farm-
#'               year panel. The clean data frame serves as the input matrix for
#'               downstream statistical and machine-learning pipelines that
#'               predict inspection pass/fail outcomes and analyse compliance-
#'               risk drivers. All steps are reproducible, although access to
#'               the data is strictly limited to FOAG collaborators.
#' =============================================================================
...
#' =============================================================================
#' CREATE NON-REVERSIBLE HMAC-KEYS
#' -----------------------------------------------------------------------------
#' Function to take in a data frame with farm level IDs and years and create a
#' non-reversible farm-year identifier using HMAC-BLAKE2B (see
#' <https://en.wikipedia.org/wiki/BLAKE_(hash_function)#BLAKE2>). The farm-year-
#' ID is subsequently used to connect farm-year data from different sources.
#'
#' @param data            Data frame containing the source columns.
#' @param identifier      Name of the (AGIS) identifier column.
#' @param year            Name of the year column.
#' @param varname         Destination column (default "ID").
#' @param secret          32+ random bytes used as HMAC key, by default, this is
#'                        looked up in `~/SECRET.env`.
#' =============================================================================
...
assign_identifier <- function(
  ...
  if (base::is.null(secret)) {
    ...
    if (!base::nzchar(secret))
      stop(
        "Provide a secret key via `secret` or environment variable SECRET.env",
        call. = FALSE
      )
  }
  ...
  #' `openssl::sha256()` with `key` performs HMAC
...
#' -----------------------------------------------------------------------------
#' @param data            Data frame containing the source columns.
#' @param variable        Character vector with the names of variables to be
#'                        turned to factor variables.
#' =============================================================================

assign_factor_variables <- function(data, variables) {
  for (i in variables) {
...
#' =============================================================================
#' PARSE ANNOYING NUMBER FORMAT
#' -----------------------------------------------------------------------------
#' Function to convert columns in a data frame to numeric and re-format query
#' artifacts (i.e. remove the apostrophe in large numbers like 100'000)
#'
#' @param data            Data frame containing the source columns.
#' @param variable        Character vector with the names of variables to be
#'                        turned to numeric variables.
#' =============================================================================

assign_numeric_variables <- function(data, variables) {
  for (i in variables) {
...
#' -----------------------------------------------------------------------------
#' Here, we read the CSV table containing inspection results queried from
#' ACONTROL, assign new variable names, change the formatting of columns for
#' easier use in R and assign new pseudonymous IDs.
#' =============================================================================

#' Read the data from the CSV file
inspections <- readr::read_csv(
  file = "data/inspections.csv",
...
inspections <- assign_identifier(inspections, "AGIS Nummer", "year")

#' Simplify the results to a binary value with 0 = Pass, 1 = Fail. The rationale
#' for this is that the values, i.e. number of passes/fails per farm are not
#' really comparable, as they depend on the context of the inspection, the div-
#' ersity of the farm etc. Binary outcomes simplify the problem to the question
#' of actual interest: Is a farm at risk of not passing an inspection?
...
inspections <- subset(
  ...
  assign_factor_variables(variables = c("theme", "type", "year", "farm"))
...
agis <- readr::read_csv("data/agis.csv", show_col_types = FALSE) %>%
  ...
  assign_identifier("BBS ID (as number)", "Jahr") %>%

  #' Assign special identifier for HODUFLU-AGIS join
  assign_identifier("KT_ID_B", "Jahr", "KTID") %>%
...
agis <- agis %>%
  ...
  assign_numeric_variables(c("LN", "SAK", "GVE"))
...
#' -----------------------------------------------------------------------------
#' Only AGIS contains all necessary IDs to connect the data, so we construct an
#' intermediate data frame `hoduflagis` and subsequently join that with the
#' previously constructed data frame `inspections`
#' =============================================================================
...
hoduflu <- readr::read_csv("data/hoduflu.csv", show_col_types = FALSE) %>%
  ...
  dplyr::rename(
    ...
    recDeliv = "Recyclingdünger Anz. Lieferungen tot",
...
area <- readr::read_delim(
  ...
  #' Assign identifier for matching
  assign_identifier("BBS_ID", "Jahr") %>%

  #' Assign abbreviated English names for ease of use
  dplyr::rename(
    elements           = "Andere Elemente", #' stone walls, hedges, …
    trees              = "Bäume",
    nonLNarea          = "Flächen ausserhalb der LN", #' outside farmland
    ...
    otherLNarea        = "Weitere Flächen innerhalb der LN"  #' other farmland
  ) %>%
...
crops <- read_csv("data/crops.csv", show_col_types = FALSE) %>%
  ...
  assign_numeric_variables("ha")

#' Mapping for crop group names. In the query, crops were aggregated to
#' agronomically similar groups.
crop_names <- c(
...
#' Compute the relative (%) share of each crop instead of the area of the crop.
#' The goal here is to have similar values for farms with similar activities
#' independent on farm size.
for (i in crop_names) {
...
#' Mapping for animal group names. In the query, animals were aggregated to
#' agronomically similar groups.
animal_names <- c(
...
#' Function to read an animal table (the data is split into two separate tables,
#' `animals_GJB.csv` and `animals_SB.csv`). Both tables can be parsed using the
#' same function.
read_animals <- function(path) {
  ...
  readr::read_delim(path, show_col_types = FALSE) %>%
    ...
    assign_numeric_variables(names(animal_names))
}
